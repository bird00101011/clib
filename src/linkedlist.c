...
__declspec(dllexport) int LinkedList_insert(LPLinkedList lp_ll, long pos,
  ...
  if (lp_ll->eles_num == 0) {
    ...
    if (pos - abs(pos - lp_ll->eles_num) < 0) {
      lp_lln = lp_ll->lp_head;
      for (i = 1; i <= pos; i++) lp_lln = lp_lln->next;
    } else {
      lp_lln = lp_ll->lp_tail;
      for (i = lp_ll->eles_num - 2; i >= pos; i--) lp_lln = lp_lln->prev;
    }
...
__declspec(dllexport) int LinkedList_del_by_ele(LPLinkedList lp_ll, void* ele,
  ...
  if (lp_ll->eles_num == 0) return FALSE;
...
__declspec(dllexport) int LinkedList_get_by_pos(LPLinkedList lp_ll, long pos,
  ...
  long i;
  if (pos - abs(pos - lp_ll->eles_num) < 0) {
    for (i = 0, lp_lln = lp_ll->lp_head; i < pos; i++) lp_lln = lp_lln->next;
  } else {
