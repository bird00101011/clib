...
__declspec(dllexport) int DynaArray_reallocate(LPDynaArray lp_da,
                                               long new_capacity) {
  assert(lp_da != NULL_POINTER);
  if (new_capacity <= 0) {
    set_last_error(CLIB_PARAMS_WRONG);
...
__declspec(dllexport) int DynaArray_del_by_ele(LPDynaArray lp_da, void* ele,
  ...
  for (i = 0, count = 0; i < lp_da->eles_num; i++, count++) {
    ...
    if (lp_da->compare_func == NULL_POINTER) {
      if (memcmp(src, dst, lp_da->ele_size) == 0) eq = TRUE;
    } else
      eq = lp_da->compare_func(src, dst);

    if (eq == TRUE) {
      if (DynaArray_del_by_pos(lp_da, i) == FALSE) return FALSE;
...
__declspec(dllexport) int DynaArray_edit_by_ele(LPDynaArray lp_da,
  ...
  for (long i = 0; i < lp_da->eles_num; i++) {
    src = arr + i * lp_da->ele_size;
    if (lp_da->compare_func == NULL_POINTER) {
      if (memcmp(src, oe, lp_da->ele_size) == 0) eq = TRUE;
    } else
      eq = lp_da->compare_func(src, oe);

    if (eq == TRUE) {
      if (lp_da->copy_func == NULL_POINTER) {
        if (memcpy(src, new_ele, lp_da->ele_size) != 0) return FALSE;
      } else {
...
__declspec(dllexport) int DynaArray_get_by_pos(LPDynaArray lp_da, long pos,
  ...
  if (lp_da->copy_func == NULL_POINTER) {
    ...
    if (lp_da->copy_func(oe, src) == FALSE) return FALSE;
  }
